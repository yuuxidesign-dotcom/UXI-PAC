...
// ── 評估量表徽章（5格獨立顯示：初評／複評1-3／結案評估）：c.assessments 各節點含 done／date，逾期變紅先不實作 ──
const ASSESSMENT_STAGES=[['initial','初'],['f1','複1'],['f2','複2'],['f3','複3'],['close','結案']];
function assessmentBadges(c){
  const a=c.assessments||{};
  const badges=ASSESSMENT_STAGES.map(([key,label])=>`<span class="badge ${a[key]?.done?'badge-green':'badge-gray'}" style="font-size:10px;padding:1px 5px">${label}</span>`).join('');
...
function dualPaneListItem(c,tabKey){
  ...
  return `<div onclick="selectListCase('${tabKey}','${c.id}')" style="padding:8px 9px;border-radius:6px;cursor:pointer;${selected?'background:var(--blue-light);border:1px solid var(--blue-mid)':'border:1px solid transparent'}">
    ...
    <div style="font-size:10.5px;color:var(--gray-500);margin-top:2px">${c.mode}・${c.disease}</div>
...
// 26 欄分 4 組（中間以較粗欄框線區隔，技術上仍是同一張表格）：
// 第1組｜基本狀態（9）：姓名～展延
// 第2組｜臨床資訊＋結案資訊（6）：入院時管路～結案是否成功（開案日於此組刻意重複顯示，供結案審視時對照）
...
function renderArchiveTable(cases,emptyMsg){
  ...
  const headers=['年度編號','姓名','病歷號碼','身分證','PAC疾病別','照護模式','開案日','預計結案日','照護進度','展延',
    ...
    '初評','複評1','複評2','複評3',
    '負責人','操作'];
  const th=(h,i)=>`<th style="${ARCHIVE_GROUP_END.has(i)?'border-right:2px solid var(--gray-300)':''}">${h}</th>`;
  return `
  <div style="overflow-x:auto">
    <table class="case-list-table archive-table" style="min-width:2200px">
      <thead>
        <tr>${headers.map((h,i)=>th(h,i)).join('')}</tr>
      </thead>
      <tbody>
...
function renderArchiveRow(c){
  ...
  const extWeeksText=(()=>{
    const period=PAC_CARE_PERIOD[c.diseaseCategory||c.disease];
    if(!c.week||!period) return '—';
    return `${Math.max(0,c.week-period.weeksMin)}週`;
  })();
  const outcomeCls=(c.archiveOutcome||'').startsWith('結案成功')?'badge-green':(c.archiveOutcome||'').startsWith('結案失敗')?'badge-red':'badge-gray';
  const a=c.assessments||{};
  ...
  const cells=[
    ...
    c.openDate||'—',
    c.archiveDate||'—',
    `<span class="badge ${outcomeCls}">${c.archiveOutcome||'—'}</span>`,
    c.dischargeDest2||'—',
    c.mode==='住院'?(c.hospitalDays!=null?`${c.hospitalDays}天`:'—'):'—',
    extWeeksText,
    c.patientDest||'—',
    c.homeVisitDate||'—',
    c.homeVisitStaff||'—',
...
function renderDetail(container,caseId){
  ...
  // 總覽／後續處置 Tab：備註編輯狀態、後續處置欄位唯讀狀態
  const overviewNoteEditing=isMgr&&overviewNoteEditCaseId===c.id;
  const dispositionReadonly=(!isMgr)||c.status==='封存';
  const dispositionNoteEditing=isMgr&&dispositionNoteEditCaseId===c.id;
  // 總覽 Tab：展延週數（僅展延後階段顯示，公式與 extensionBadge 一致並做防呆下限 0）
  const extWeeksText=(()=>{
    if(c.timelineSub!=='展延後') return '—';
    const period=PAC_CARE_PERIOD[c.diseaseCategory||c.disease];
    const base=period?period.weeksMin:c.week;
    return `${Math.max(0,c.week-base)}週`;
  })();
  // 詳情頁標題徽章：改用展延狀態（而非單純的 c.status）呈現，與總覽 Tab 的「展延」欄位語意一致
  const extBadge=extensionBadge(c);
  ...
  container.innerHTML=`
    ...
    <div class="detail-header">
      <div class="detail-top">
        <div class="patient-name">
          ...
          <span class="badge ${extBadge.cls}">${extBadge.text}</span>
      ...
      <div class="detail-meta">
        ...
        <div class="meta-item"><strong>預計結案日：</strong>${c.closeDate||'—'}</div>
    ...
    <div class="detail-tab-panel" data-tab-key="overview" style="${tabPanelStyle('overview')}">
      ...
      <div class="section-card">
        ...
        <div class="sc-body">
          ...
          <div class="info-grid">
            <div class="info-item"><label>開案日</label><span>${c.openDate||'—'}</span></div>
            <div class="info-item"><label>預計結案日</label><span>${c.closeDate||'—'}</span></div>
            <div class="info-item"><label>照護進度</label><span>${careProgressText(c)}</span></div>
            <div class="info-item"><label>展延</label><span>${extensionBadge(c).text}</span></div>
            <div class="info-item"><label>展延週數</label><span>${extWeeksText}</span></div>
          </div>
          <div class="divider"></div>
          <div class="info-grid">
            <div class="info-item"><label>轉介來源</label><span>${c.source}</span></div>
            <div class="info-item"><label>入院時管路</label><span>${c.admissionTubes||'—'}</span></div>
            <div class="info-item"><label>發病時間</label><span>${c.onsetDate||'—'}</span></div>
            <div class="info-item"><label>ICD</label><span>${c.icdCode||'—'}</span></div>
          </div>
          <div class="divider"></div>
          <div class="info-grid">
      ...
      <div class="section-card">
        <div class="sc-header">
          <div class="sc-title">📝 備註</div>
          ${isMgr?(overviewNoteEditing
            ?`<button class="btn btn-primary btn-xs" onclick="saveOverviewNote('${c.id}')">💾 儲存</button>`
            :`<button class="btn btn-ghost btn-xs" onclick="toggleOverviewNoteEdit('${c.id}')">✏️ 編輯</button>`):''}
        </div>
        <div class="sc-body">
          ${overviewNoteEditing
            ?`<textarea class="form-control" id="overview-note-input" rows="4" style="width:100%" placeholder="輸入備註內容…">${c.overviewNote||''}</textarea>`
            :`<div style="font-size:13px;color:${c.overviewNote?'var(--gray-700)':'var(--gray-400)'};white-space:pre-wrap">${c.overviewNote||'尚無備註'}</div>`}
        </div>
      </div>
    </div>
    ...
    <div class="detail-tab-panel" data-tab-key="assessment" style="${tabPanelStyle('assessment')}">
      ...
      <div class="section-card">
        <div class="sc-header"><div class="sc-title">評估量表區（未來串接評估量表模組）</div></div>
        <div class="sc-body">
          <div style="border:1px dashed var(--gray-300);border-radius:8px;padding:30px;text-align:center;color:var(--gray-400);font-size:12px;background:var(--gray-50)">尚未建置，敬請期待</div>
        </div>
      </div>
    </div>
    ...
    <!-- 後續處置：出院去向／病人去向快選＋家訪資訊＋備註，取代原本的「轉介」Tab -->
    <div class="detail-tab-panel" data-tab-key="disposition" style="${tabPanelStyle('disposition')}">
      <div class="section-card">
        <div class="sc-header">
          <div class="sc-title">🔄 後續處置</div>
          <span class="badge ${referralBadge(c).cls}">${referralBadge(c).text}</span>
        </div>
        <div class="sc-body">
      ...
      <div class="section-card">
        <div class="sc-header">
          <div class="sc-title">📝 備註</div>
          ${isMgr?(dispositionNoteEditing
            ?`<button class="btn btn-primary btn-xs" onclick="saveDispositionNote('${c.id}')">💾 儲存</button>`
            :`<button class="btn btn-ghost btn-xs" onclick="toggleDispositionNoteEdit('${c.id}')">✏️ 編輯</button>`):''}
        </div>
        <div class="sc-body">
          ${dispositionNoteEditing
            ?`<textarea class="form-control" id="disposition-note-input" rows="4" style="width:100%" placeholder="輸入備註內容…">${c.dispositionNote||''}</textarea>`
            :`<div style="font-size:13px;color:${c.dispositionNote?'var(--gray-700)':'var(--gray-400)'};white-space:pre-wrap">${c.dispositionNote||'尚無備註'}</div>`}
        </div>
      </div>
    </div>

    <!-- 病摘與PAC判斷：比照收案管理彈窗呈現，個案已是正式病歷、判斷已完成，故改唯讀顯示歷史紀錄 -->
    <div class="detail-tab-panel" data-tab-key="judge" style="${tabPanelStyle('judge')}">
      <div class="section-card">
        <div class="sc-header"><div class="sc-title">🩺 PAC 收案判斷</div></div>
        <div class="sc-body">
          ${c.judgeRecord?(()=>{
            const jr=c.judgeRecord;
            return `
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:14px">
            ${['是PAC','非PAC','需再評估'].map(opt=>`<div style="padding:10px 14px;border:2px solid ${jr.result===opt?'var(--blue)':'var(--gray-200)'};border-radius:8px;background:${jr.result===opt?'var(--blue-light)':'var(--white)'};font-size:13px;font-weight:${jr.result===opt?'700':'500'};color:${jr.result===opt?'var(--blue)':'var(--gray-400)'};text-align:center">${jr.result===opt?'✓ ':''}${opt}</div>`).join('')}
          </div>
          <div class="info-grid">
            <div class="info-item"><label>判斷PAC疾病別</label><span>${jr.diseaseCategory||'—'}</span></div>
            <div class="info-item"><label>判斷者</label><span>${jr.judgedBy||'—'}</span></div>
          </div>
          <div class="divider"></div>
          <div class="info-item" style="margin-bottom:10px"><label>判斷原因</label><span>${jr.reason||'—'}</span></div>
          <div class="info-item"><label>補充建議</label><span>${jr.suggestion||'—'}</span></div>
          `;})():`<div style="text-align:center;padding:20px;color:var(--gray-400);font-size:12px">尚無判斷紀錄</div>`}
        </div>
      </div>
    </div>
...
// ── 總覽 Tab「備註」編輯 ──
function toggleOverviewNoteEdit(caseId){
  overviewNoteEditCaseId=caseId;
  renderPage('detail',currentCase);
}
function saveOverviewNote(caseId){
  const c=getCurrentCaseObj();
  const input=document.getElementById('overview-note-input');
  if(c) c.overviewNote=(input?.value||'').trim();
  overviewNoteEditCaseId=null;
  renderPage('detail',currentCase);
}
...
function saveDispositionNote(caseId){
  const c=getCurrentCaseObj();
  const input=document.getElementById('disposition-note-input');
  if(c) c.dispositionNote=(input?.value||'').trim();
  dispositionNoteEditCaseId=null;
  renderPage('detail',currentCase);
}
